from etf_other_tabs_scraper import getRiskNRating, recordStatsOnDb, retry_count, addToDB
...
if __name__ == '__main__':
    recordStatsOnDb(f"ETF Risk n Rating Started",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    sec_ids = open(f'../../daily/etf/etf_sec_ids.txt', 'r').read().split('\n')
    with ThreadPoolExecutor(max_workers=3) as executor:
        executor.map(processInThread, sec_ids)
    recordStatsOnDb(f"ETF Risk n Rating Completed",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    recordStatsOnDb(f"ETF Risk n Rating Processed", processed)
    recordStatsOnDb(f"ETF Risk n Rating Empty", empty_records)
