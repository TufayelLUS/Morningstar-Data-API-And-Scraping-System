from etf_other_tabs_scraper import getOverview, recordStatsOnDb, retry_count, addToDB
        ...
        try:
            ...
            if overview.get('Flag'):
                if overview.get('Flag') == 'Deleted':
                    addToDB(f'etf_overview', overview, True)
                else:
                    addToDB(f'etf_overview', overview)
                break
...
if __name__ == '__main__':
    recordStatsOnDb(f"ETF Overview Started",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    sec_ids = open(f'../../daily/etf/etf_sec_ids.txt', 'r').read().split('\n')
    with ThreadPoolExecutor(max_workers=3) as executor:
        executor.map(processInThread, sec_ids)
    recordStatsOnDb(f"ETF Overview Completed",
                    datetime.now().strftime(
                        '%Y-%m-%d %H:%M:%S'))
    recordStatsOnDb(f"ETF Overview Processed", processed)
    recordStatsOnDb(f"ETF Overview Empty", empty_records)
