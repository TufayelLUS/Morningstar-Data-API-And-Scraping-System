from life_other_tabs_scraper import getPortfolio, recordStatsOnDb, input_file_path, threads_count, retry_count, addToDB
...
if __name__ == '__main__':
    recordStatsOnDb("Life Portfolio Started",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    sec_ids = open(input_file_path, 'r').read().split('\n')
    with ThreadPoolExecutor(max_workers=2) as executor:
        executor.map(processInThread, sec_ids)
    recordStatsOnDb("Life Portfolio Completed",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    recordStatsOnDb("Life Portfolio Processed", processed)
    recordStatsOnDb("Life Portfolio Empty", empty_records)
