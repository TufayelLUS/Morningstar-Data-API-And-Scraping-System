...
# input_file_path = 'pension_sec_ids.txt'
input_file_path = '../../daily/pension_life/pension_sec_ids.txt'
        ...
        if not table:
            ...
            for row in rows:
                ...
                if left_cell and right_cell:
                    if object_type == 'dict':
                        container_obj[left_cell.text.strip()] = right_cell.text.strip()
                    else:
                        container_obj.append({
...
def getDocuments(SecId):
    ...
    link = f'https://lt.morningstar.com/3y3wd9echv/snapshot/default.aspx?tab=11&SecurityToken={SecId}%5D9%5D0%5DSAGBR%24%24PSA&Id={SecId}&ClientFund=0&CurrencyId=GBP'
    loaded = False
...
def getRiskNRating(SecId):
    ...
    link = f'https://lt.morningstar.com/3y3wd9echv/snapshot/default.aspx?tab=2&SecurityToken={SecId}%5D9%5D0%5DSAGBR%24%24PSA&Id={SecId}&ClientFund=0&CurrencyId=GBP'
    loaded = False
    for i in range(retry_count):
        proxy = pickProxy()
        proxies = {
            ...
            for row in rows:
                if row.parent.name == 'thead':
                    ...
                    for cell in all_cells:
                        if cell.text.strip() == '':
                            continue
                        item_headers.append(cell.get_text(separator='\n').strip().replace('\n', ' '))
                else:
                    cell_title = row.find('th')
                    cells = row.find_all('td')
                    row_data = []
...
def getPerformance(SecId):
    ...
    link = f'https://lt.morningstar.com/3y3wd9echv/snapshot/default.aspx?tab=1&SecurityToken={SecId}%5D9%5D0%5DSAGBR%24%24PSA&Id={SecId}&ClientFund=0&CurrencyId=GBP'
    loaded = False
    for i in range(retry_count):
        proxy = pickProxy()
        proxies = {
        ...
        if not table:
            ...
            for row in rows:
                if row.parent.name == 'thead':
                    ...
                    for cell in cells:
                        if cell.find('img'):
                            row_data.append("https://lt.morningstar.com" + cell.find('img').get('src'))
...
def getManagement(SecId):
    ...
    link = f'https://lt.morningstar.com/3y3wd9echv/snapshot/default.aspx?tab=4&SecurityToken={SecId}%5D9%5D0%5DSAGBR%24%24PSA&Id={SecId}&ClientFund=0&CurrencyId=GBP'
    loaded = False
    for i in range(retry_count):
        proxy = pickProxy()
        proxies = {
...
def getOverview(SecId):
    ...
    link = f'https://lt.morningstar.com/3y3wd9echv/snapshot/default.aspx?tab=0&SecurityToken={SecId}%5D9%5D0%5DSAGBR%24%24PSA&Id={SecId}&ClientFund=0&CurrencyId=GBP'
    loaded = False
    for i in range(retry_count):
        proxy = pickProxy()
        proxies = {
    ...
    if morningstar_rating_container:
        try:
            final_data['MorningstarRating'] = morningstar_rating_container.find('img').get('src').split('/')[-1].split('.')[0]
        except:
            pass
    esg_rating_container = soup.find('div', id='MorningstarSustainabilityRating')
    if esg_rating_container:
        try:
            final_data['ESGRating'] = esg_rating_container.find('img').get('alt')
        except:
            pass
    morningstar_medalist_rating_container = soup.find('div', id='MorningstarMedalistRating')
    ...
    if srri_container:
        try:
            final_data['SRRI'] = srri_container.find('li', {'class': 'selected'}).text.strip()
        except:
            ...
            for row in rows:
                if row.parent.name == 'thead':
                    ...
                    for cell in cells:
                        # if cell.text.strip() == '':
                        #     continue
                        row_data.append(cell.get_text(separator='\n').strip().replace('\n', ' '))
                    if not row_data:
                        continue
                    top_five_holdings_data.append(
                        dict(zip(item_headers, row_data)))
            # remove incomplete data
            top_five_holdings_data = [x for x in top_five_holdings_data if len(x) == len(item_headers)]
    portfolio_overview_data['Top 5 Holdings'] = top_five_holdings_data
    fixed_income_sector_container = soup.find('div', {'id': 'TopFixedIncomeSectors'})
...
def getAdditionalOverview(SecId):
    link = f'https://www.us-api.morningstar.com/sal/sal-service/fund/esg/v1/{SecId}/data?languageId=en-GB&locale=en-GB&clientId=EC&benchmarkId=category&component=sal-sustainability&version=4.13.0'
...
def getPortfolio(SecId):
    ...
    link = f'https://lt.morningstar.com/3y3wd9echv/snapshot/default.aspx?tab=3&SecurityToken={SecId}%5D9%5D0%5DSAGBR%24%24PSA&Id={SecId}&ClientFund=0&CurrencyId=GBP'
    loaded = False
    for i in range(retry_count):
        proxy = pickProxy()
        proxies = {
    ...
    maturity_distribution_container = soup.find('div', id='MaturityDistribution')
        ...
        if not table:
            ...
            for row in rows:
                if row.parent.name == 'thead':
                    ...
                    if not row_data:
                        continue
                    maturity_distribution_data.append(dict(zip(item_headers, row_data)))
        ...
        if not table:
            ...
            for row in rows:
                if row.parent.name == 'thead':
                    ...
                    if not row_data:
                        continue
                    maturity_summary_data.append(dict(zip(item_headers, row_data)))
    final_data['Maturity Summary'] = maturity_summary_data
    final_data['Credit Quality Breakdown'] = getPageSectionDynamic(soup, 'CreditQuality')
    final_data['Fixed Income Sector Weightings'] = getPageSectionDynamic(soup, 'FixedIncomeSectorWeightings')
    final_data['Asset Allocation'] = getPageSectionDynamic(soup, 'AssetAllocationNetTable')
    final_data['Holdings']['Summary'] = getPageSectionLeftRight(soup, 'TopHoldingsSummary', 'list')
    final_data['Exposure']['Region'] = getPageSectionDynamic(soup, 'WorldRegions', custom_parameter_name='Region')
    try:
    ...
    final_data['Exposure']['Sector'] = getPageSectionDynamic(soup, 'StockSectorWeightings', custom_parameter_name='Sector')
    try:
            ...
            try:
                ...
                if overview.get('Flag'):
                    if overview.get('Flag') == 'Deleted':
                        addToDB('pension_overview', overview, True)
                    else:
                        addToDB('pension_overview', overview)
                    break
            ...
            try:
                ...
                if risk_n_rating.get('Flag'):
                    if risk_n_rating.get('Flag') == 'Deleted':
                        addToDB('pension_risk_n_rating', risk_n_rating, True)
                    else:
                        addToDB('pension_risk_n_rating', risk_n_rating)
                    break
            ...
            try:
                ...
                if performance.get('Flag'):
                    if performance.get('Flag') == 'Deleted':
                        addToDB('pension_performance', performance, True)
                    else:
                        addToDB('pension_performance', performance)
                    break
            ...
            try:
                ...
                if management.get('Flag'):
                    if management.get('Flag') == 'Deleted':
                        addToDB('pension_management', management, True)
                    else:
                        addToDB('pension_management', management)
                    break
