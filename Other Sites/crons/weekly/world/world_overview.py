from world_other_tabs_scraper import getOverview, recordStatsOnDb, retry_count, addToDB
        ...
        try:
            ...
            if overview.get('Flag'):
                if overview.get('Flag') == 'Deleted':
                    addToDB(f'{country_code_db_name}_overview', overview, True)
                else:
                    addToDB(f'{country_code_db_name}_overview', overview)
                break
...
if __name__ == '__main__':
    ...
    recordStatsOnDb(f"World {country_code.upper()} Overview Started",
    ...
    recordStatsOnDb(f"World {country_code.upper()} Overview Completed",
                    datetime.now().strftime(
                        '%Y-%m-%d %H:%M:%S'))
    recordStatsOnDb(f"World {country_code.upper()} Overview Processed", processed)
    recordStatsOnDb(f"World {country_code.upper()} Overview Empty", empty_records)
