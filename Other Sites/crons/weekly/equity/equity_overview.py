from equity_other_tabs_scraper import getOverview, recordStatsOnDb, retry_count, addToDB, input_file_path
...
def processInThread(SecId):
    ...
    for i in range(retry_count):
        try:
            ...
            if not overview.get('keyStats') and not overview.get('CompanyProfile') and not overview.get('Financials') and i < retry_count - 1:
                continue
            print(json.dumps(overview, indent=4))
            # check in the last loop if we have missing
            if i == retry_count - 1 and (not overview.get('keyStats') and not overview.get('CompanyProfile') and not overview.get('Financials')):
...
if __name__ == '__main__':
    recordStatsOnDb(f"Equity Overview Started",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    sec_ids = open(input_file_path, 'r').read().split('\n')
    with ThreadPoolExecutor(max_workers=8) as executor:
        executor.map(processInThread, sec_ids)
    recordStatsOnDb(f"Equity Overview Completed",
                    datetime.now().strftime(
                        '%Y-%m-%d %H:%M:%S'))
    recordStatsOnDb(f"Equity Overview Processed", processed)
    recordStatsOnDb(f"Equity Overview Empty", empty_records)
