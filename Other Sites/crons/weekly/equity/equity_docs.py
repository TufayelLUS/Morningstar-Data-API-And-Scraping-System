from equity_other_tabs_scraper import getDocuments, recordStatsOnDb, input_file_path, retry_count, addToDB
...
if __name__ == '__main__':
    recordStatsOnDb("Equity Docs Started",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    sec_ids = open(input_file_path, 'r').read().split('\n')
    with ThreadPoolExecutor(max_workers=8) as executor:
        executor.map(processInThread, sec_ids)
    recordStatsOnDb("Equity Docs Completed",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    recordStatsOnDb("Equity Docs Processed", processed)
    recordStatsOnDb("Equity Docs Empty", empty_records)
