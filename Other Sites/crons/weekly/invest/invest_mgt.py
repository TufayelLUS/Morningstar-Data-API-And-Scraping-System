from invest_other_tabs_scraper import getManagement, recordStatsOnDb, retry_count, addToDB
        ...
        try:
            ...
            if management.get('Flag'):
                if management.get('Flag') == 'Deleted':
                    addToDB(f'invest_management', management, True)
                else:
                    addToDB(f'invest_management', management)
                break
...
if __name__ == '__main__':
    recordStatsOnDb(f"Invest Management Started",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    sec_ids = open(f'../../daily/invest/invest_sec_ids.txt', 'r').read().split('\n')
    with ThreadPoolExecutor(max_workers=3) as executor:
        executor.map(processInThread, sec_ids)
    recordStatsOnDb(f"Invest Management Completed",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    recordStatsOnDb(f"Invest Management Processed", processed)
    recordStatsOnDb(f"Invest Management Empty", empty_records)
