from invest_other_tabs_scraper import getPortfolio, recordStatsOnDb, retry_count, addToDB
        ...
        try:
            ...
            if portfolio_data.get('Flag'):
                if portfolio_data.get('Flag') == 'Deleted':
                    addToDB(f'invest_portfolio', portfolio_data, True)
                else:
                    addToDB(f'invest_portfolio', portfolio_data)
                break
...
if __name__ == '__main__':
    recordStatsOnDb(f"Invest Portfolio Started",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    sec_ids = open(f'../../daily/invest/invest_sec_ids.txt', 'r').read().split('\n')
    with ThreadPoolExecutor(max_workers=3) as executor:
        executor.map(processInThread, sec_ids)
    recordStatsOnDb(f"Invest Portfolio Completed",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    recordStatsOnDb(f"Invest Portfolio Processed", processed)
    recordStatsOnDb(f"Invest Portfolio Empty", empty_records)
