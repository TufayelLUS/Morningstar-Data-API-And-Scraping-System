...
def getChartTimeSeries(id, performanceType):
    link = f'https://lt.morningstar.com/api/rest.svc/timeseries_ohlcv/3y3wd9echv'
    params = {
        'currencyId': 'GBP',
        'idtype': 'Morningstar',
        'frequency': 'daily',
...
if __name__ == '__main__':
    recordStatsOnDb("Equity Chart Daily Started",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    sec_ids = open('equity_sec_ids.txt', 'r').read().split('\n')
    with ThreadPoolExecutor(max_workers=threads_count) as executor:
        executor.map(processInThread, sec_ids)
    recordStatsOnDb("Equity Chart Daily Completed",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    recordStatsOnDb("Equity Chart Daily Processed", processed)
    recordStatsOnDb("Equity Chart Daily Empty", empty_records)
