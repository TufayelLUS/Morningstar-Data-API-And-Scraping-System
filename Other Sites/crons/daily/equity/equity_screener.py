...
def scrapeFunds(fund_type, collection_name, f=None):
    s = requests.Session()
    link = 'https://lt.morningstar.com/3y3wd9echv/stockquickrank/default.aspx?Universe={}&LanguageId=en-GB'.format(
        fund_type)
    headers = {
    ...
    while True:
        ...
        if page_no > 1:
            ...
            data = {
                '__EVENTTARGET': 'ctl00$ContentPlaceHolder1$msStockQuickrankControl$ddlPageSize',
                '__EVENTARGUMENT': page_no,
                'ctl00_ContentPlaceHolder1_msStockQuickrankControl_scrtmgrFundQuickrank_HiddenField': '',
                '__LASTFOCUS': '',
                '__VIEWSTATE': __VIEWSTATE,
                '__VIEWSTATEGENERATOR': __VIEWSTATEGENERATOR,
                '__VIEWSTATEENCRYPTED': '',
                '__EVENTVALIDATION': __EVENTVALIDATION,
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$msTopBar$ddlTitleBarCurrency': 'BAS',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$msTopBar$ddlTitleBarLanguage': 'en-GB',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$msTopBar$ddlTitleBarTools': '',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$ddlUniverse': fund_type,
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$ddlEquitySector': '',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$ddlEquityIndustry': '',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$txtSearchKey': '',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$hdnFilterBySelection': '',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$ddlPageSize': '500',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$hdnSelectedFunds': '',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$hdnSelectedIndex': '',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$hdnFirstFundNavDate': '',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$hdnCustomImageFileIds': '',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$hdnSelectedSecurityCount': '0',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$hdnTabs': 'Snapshot,ShortTerm,LongTerm,Performance,Valuation',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$msExportToExcel$hdnSelectedRow': '',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$msExportToExcel$hdnCheckedColumns': '',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$msExportToExcel$hdnExportLimit': '5000',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$msExportToExcel$hdnExportCount': '0',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$msExportToExcel$toExport': '',
                'ctl00$ContentPlaceHolder1$msStockQuickrankControl$msExportToExcel$hdnAllowAllFundsExport': 'true',
                'ctl00$__RequestVerificationToken': rv_token
            }
            if not ready:
                page_no -= 1
                print("Setting page count to 500")
                data['__EVENTTARGET'] = 'ctl00$ContentPlaceHolder1$msStockQuickrankControl$ddlPageSize'
                data['__EVENTARGUMENT'] = ''
                ready = True
            else:
                data['__EVENTTARGET'] = 'ctl00$ContentPlaceHolder1$msStockQuickrankControl$AspNetPager'
            ...
            headers = {
                ...
                'priority': 'u=0, i',
                'referer': 'https://lt.morningstar.com/3y3wd9echv/stockquickrank/default.aspx?Universe={}&LanguageId=en-GB'.format(fund_type),
                'sec-ch-ua': '"Google Chrome";v="135", "Not-A.Brand";v="8", "Chromium";v="135"',
                'sec-ch-ua-mobile': '?0',
                'sec-ch-ua-platform': '"Windows"',
                'sec-fetch-dest': 'document',
        ...
        soup = bs(resp, 'html.parser')
        table = soup.find(
            'table', {'id': 'ctl00_ContentPlaceHolder1_msStockQuickrankControl_gridResult'})
        if not table:
    ...
    while True:
        ...
        if page_no > 1:
            ...
            headers = {
                ...
                'origin': 'https://lt.morningstar.com',
                'priority': 'u=0, i',
                'referer': 'https://lt.morningstar.com/3y3wd9echv/stockquickrank/default.aspx',
                'sec-ch-ua': '"Google Chrome";v="135", "Not-A.Brand";v="8", "Chromium";v="135"',
                'sec-ch-ua-mobile': '?0',
                'sec-ch-ua-platform': '"Windows"',
                'sec-fetch-dest': 'document',
...
def equityThread(fund_type, file_handle):
    try:
        scrapeFunds(fund_type, 'equity_screener_data', file_handle)
    except:
        print(f"Failed to scrape Equity funds due to an exception")
        with open('errors.txt', 'a') as f:
            f.write("[{}] Exception: {}".format(datetime.now().strftime(
                '%Y-%m-%d %H:%M:%S'), traceback.format_exc()) + '\n')
    try:
        scrapeOtherTabs('ShortTerm', fund_type, 'equity_screener_data')
    except:
        ...
        with open('errors.txt', 'a') as f:
            f.write("[{}] Exception: {}".format(datetime.now().strftime(
                '%Y-%m-%d %H:%M:%S'), traceback.format_exc()) + '\n')
    try:
        scrapeOtherTabs('LongTerm', fund_type, 'equity_screener_data')
    except:
        print(f"Failed to scrape long term tab due to an exception")
        with open('errors.txt', 'a') as f:
            f.write("[{}] Exception: {}".format(datetime.now().strftime(
                '%Y-%m-%d %H:%M:%S'), traceback.format_exc()) + '\n')
    try:
        scrapeOtherTabs('Performance', fund_type, 'equity_screener_data')
    except:
        ...
        with open('errors.txt', 'a') as f:
            f.write("[{}] Exception: {}".format(datetime.now().strftime(
                '%Y-%m-%d %H:%M:%S'), traceback.format_exc()) + '\n')
    try:
        scrapeOtherTabs('Valuation', fund_type, 'equity_screener_data')
    except:
        print(f"Failed to scrape valuation tab due to an exception")
        with open('errors.txt', 'a') as f:
            f.write("[{}] Exception: {}".format(datetime.now().strftime(
                '%Y-%m-%d %H:%M:%S'), traceback.format_exc()) + '\n')
...
if __name__ == '__main__':
    recordStatsOnDb(f"Equity Screener Started",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    out_file_name = 'equity_sec_ids.txt'
    with open(out_file_name, 'w+', encoding='utf-8') as f:
        batches = [equity_list[i:i + threads]
                   for i in range(0, len(equity_list), threads)]
        for batch in batches:
            threads_list = []
            for item in batch:
                thread = Thread(target=equityThread, args=(item, f))
                thread.start()
                threads_list.append(thread)
            for thread in threads_list:
                thread.join()
    recordStatsOnDb(f"Equity Screener Completed",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
