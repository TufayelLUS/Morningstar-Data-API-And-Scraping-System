...
def getChartText(SecIds, fund_type):
    ...
    loaded = False
    link = f'https://lt.morningstar.com/3y3wd9echv/etfreport/default.aspx?1=1&tab=7&SecurityToken={SecIds}%5D22%5D0%5D{fund_type}&Id={SecIds}&ClientFund=0&CurrencyId=USD'
    for i in range(retry_count):
        proxy = pickProxy()
        proxies = {
            'http': proxy,
...
if __name__ == '__main__':
    recordStatsOnDb(f"ETF Chart Daily Started",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    sec_ids = open('etf_sec_ids.txt', 'r').read().split('\n')
    with ThreadPoolExecutor(max_workers=threads_count) as executor:
        executor.map(processInThreadWorld, sec_ids)
    recordStatsOnDb(f"ETF Chart Daily Completed",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    recordStatsOnDb("ETF Chart Daily Processed", processed)
    recordStatsOnDb("ETF Chart Daily Empty", empty_records)
