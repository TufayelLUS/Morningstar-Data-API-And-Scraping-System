...
def scrapeFunds(fund_type, collection_name, f=None):
    s = requests.Session()
    link = 'https://lt.morningstar.com/3y3wd9echv/etfquickrank/default.aspx?Universe={}&LanguageId=en-GB'.format(
        fund_type)
    headers = {
    ...
    while True:
        ...
        if page_no > 1:
            ...
            data = {
                ...
                'ctl00$ContentPlaceHolder1$aFundQuickrankControl$msTopBar$ddlTitleBarLanguage': 'en-GB',
                ...
                'ctl00$ContentPlaceHolder1$aFundQuickrankControl$hdnTabs': 'Snapshot,Performance,ShortTerm,Portfolio,FeesAndDetails,Documents',
            ...
            headers = {
                ...
                'priority': 'u=0, i',
                'referer': 'https://lt.morningstar.com/3y3wd9echv/etfquickrank/default.aspx?Universe={}&LanguageId=en-GB'.format(fund_type),
                'sec-ch-ua': '"Google Chrome";v="135", "Not-A.Brand";v="8", "Chromium";v="135"',
                'sec-ch-ua-mobile': '?0',
                'sec-ch-ua-platform': '"Windows"',
                'sec-fetch-dest': 'document',
    ...
    while True:
        ...
        if page_no > 1:
            ...
            headers = {
                ...
                'origin': 'https://lt.morningstar.com',
                'priority': 'u=0, i',
                'referer': 'https://lt.morningstar.com/3y3wd9echv/etfquickrank/default.aspx',
                'sec-ch-ua': '"Google Chrome";v="135", "Not-A.Brand";v="8", "Chromium";v="135"',
                'sec-ch-ua-mobile': '?0',
                'sec-ch-ua-platform': '"Windows"',
                'sec-fetch-dest': 'document',
...
def etfThread(fund_type, file_handle):
    recordStatsOnDb(f"ETF Screener Started",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    try:
        scrapeFunds(fund_type, 'etf_screener_data', file_handle)
    except:
        print(f"Failed to scrape ETFs due to an exception")
        with open('errors.txt', 'a') as f:
            f.write("[{}] Exception: {}".format(datetime.now().strftime(
                '%Y-%m-%d %H:%M:%S'), traceback.format_exc()) + '\n')
    try:
        scrapeOtherTabs('ShortTerm', fund_type, 'etf_screener_data')
    except:
        ...
        with open('errors.txt', 'a') as f:
            f.write("[{}] Exception: {}".format(datetime.now().strftime(
                '%Y-%m-%d %H:%M:%S'), traceback.format_exc()) + '\n')
    try:
        scrapeOtherTabs('Performance', fund_type, 'etf_screener_data')
    except:
        ...
        with open('errors.txt', 'a') as f:
            f.write("[{}] Exception: {}".format(datetime.now().strftime(
                '%Y-%m-%d %H:%M:%S'), traceback.format_exc()) + '\n')
    try:
        scrapeOtherTabs('Portfolio', fund_type, 'etf_screener_data')
    except:
        ...
        with open('errors.txt', 'a') as f:
            f.write("[{}] Exception: {}".format(datetime.now().strftime(
                '%Y-%m-%d %H:%M:%S'), traceback.format_exc()) + '\n')
    try:
        scrapeOtherTabs('FeesAndDetails', fund_type, 'etf_screener_data')
    except:
        ...
        with open('errors.txt', 'a') as f:
            f.write("[{}] Exception: {}".format(datetime.now().strftime(
                '%Y-%m-%d %H:%M:%S'), traceback.format_exc()) + '\n')
    try:
        scrapeOtherTabs('Documents', fund_type, 'etf_screener_data')
    except:
        print(f"Failed to scrape documents tab due to an exception")
        with open('errors.txt', 'a') as f:
            f.write("[{}] Exception: {}".format(datetime.now().strftime(
                '%Y-%m-%d %H:%M:%S'), traceback.format_exc()) + '\n')
    recordStatsOnDb(f"ETF Screener Completed",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))


if __name__ == '__main__':
    out_file_name = 'etf_sec_ids.txt'
    with open(out_file_name, 'w+', encoding='utf-8') as f:
        for fund_type in fund_type:
            thread = Thread(target=etfThread, args=(fund_type, f))
            thread.start()
            thread.join()
