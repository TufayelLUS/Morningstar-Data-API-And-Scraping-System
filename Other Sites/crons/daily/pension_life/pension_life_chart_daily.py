...
def getChartText(SecIds, fund_type):
    ...
    loaded = False
    link = f'https://lt.morningstar.com/3y3wd9echv/snapshot/default.aspx?tab=7&SecurityToken={SecIds}%5D9%5D0%5DSAGBR%24%24{fund_type}&Id={SecIds}&ClientFund=0&CurrencyId=GBP'
    for i in range(retry_count):
        proxy = pickProxy()
        proxies = {
            'http': proxy,
...
def processInThreadLife(SecId):
    ...
    try:
        charts = getChartText(SecId, 'LSA')
        for chart in charts:
            addToDB('life_charts', chart)
    except:
...
def processInThreadPension(SecId):
    ...
    print(f"Processing {SecId}")
    try:
        charts = getChartText(SecId, 'PSA')
        for chart in charts:
            addToDB('pension_charts', chart)
    except:
        with open('errors.txt', 'a') as f:
            f.write("[{}] SecId: {}\nException: {}".format(datetime.now().strftime(
                '%Y-%m-%d %H:%M:%S'), SecId, traceback.format_exc()) + '\n')
