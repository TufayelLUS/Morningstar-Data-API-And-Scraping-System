from funds_other_tabs_scraper import getDocuments, recordStatsOnDb, input_file_path, threads_count, retry_count, addToDB
...
if __name__ == '__main__':
    recordStatsOnDb("Funds Docs Started",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    sec_ids = open(input_file_path, 'r').read().split('\n')
    with ThreadPoolExecutor(max_workers=11) as executor:
        executor.map(processInThread, sec_ids)
    recordStatsOnDb("Funds Docs Completed",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    recordStatsOnDb("Funds Docs Processed", processed)
    recordStatsOnDb("Funds Docs Empty", empty_records)
