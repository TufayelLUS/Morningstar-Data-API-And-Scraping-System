from pension_other_tabs_scraper import getPerformance, recordStatsOnDb, input_file_path, threads_count, retry_count, addToDB
...
if __name__ == '__main__':
    recordStatsOnDb("Pension Performance Started",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    sec_ids = open(input_file_path, 'r').read().split('\n')
    with ThreadPoolExecutor(max_workers=3) as executor:
        executor.map(processInThread, sec_ids)
    recordStatsOnDb("Pension Performance Completed",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    recordStatsOnDb("Pension Performance Processed", processed)
    recordStatsOnDb("Pension Performance Empty", empty_records)
