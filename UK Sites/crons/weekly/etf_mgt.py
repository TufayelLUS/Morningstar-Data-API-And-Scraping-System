from etf_other_tabs_scraper import getManagement, recordStatsOnDb, input_file_path, threads_count, retry_count, addToDB
...
if __name__ == '__main__':
    recordStatsOnDb("ETF Management Started",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    sec_ids = open(input_file_path, 'r').read().split('\n')
    with ThreadPoolExecutor(max_workers=3) as executor:
        executor.map(processInThread, sec_ids)
    recordStatsOnDb("ETF Management Completed",
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    recordStatsOnDb("ETF Management Processed", processed)
    recordStatsOnDb("ETF Management Empty", empty_records)
