...
def cleanUpDB():
    client = MongoClient(mongo_connection_string)
    db = client['morningstar']
    collection = db['pension_charts_temp']
    collection.delete_many({})
    collection = db['life_charts_temp']
    collection.delete_many({})
    client.close()
...
def migrateNewDBToOld():
    client = MongoClient(mongo_connection_string)
    db = client['morningstar']
    collection_new_fund = db['pension_charts_temp']
    collection_new_etf = db['life_charts_temp']
    collection_old_fund = db['pension_charts']
    collection_old_etf = db['life_charts']
    # Migrate fund data
...
def processInThreadPension(SecId):
    ...
    try:
        charts = getChartText(SecId)
        for chart in charts:
            addToDBDirect('pension_charts_temp', chart)
    except:
        with open('errors.txt', 'a') as f:
            f.write("[{}] SecId: {}\nException: {}".format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'), SecId, traceback.format_exc()) + '\n')
...
if __name__ == '__main__':
    cleanUpDB()
    recordStatsOnDb("Pension Chart Daily Started", datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    sec_ids = open('pension_sec_ids.txt', 'r').read().split('\n')
    with ThreadPoolExecutor(max_workers=threads_count) as executor:
        executor.map(processInThreadPension, sec_ids)
    recordStatsOnDb("Pension Chart Daily Completed", datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    recordStatsOnDb("Pension Chart Daily Processed", processed)
    recordStatsOnDb("Pension Chart Daily Empty", empty_records)
    processed = 0
    empty_records = 0
    recordStatsOnDb("Life Chart Daily Started", datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    sec_ids = open('life_sec_ids.txt', 'r').read().split('\n')
    with ThreadPoolExecutor(max_workers=threads_count) as executor:
        executor.map(processInThreadLife, sec_ids)
    recordStatsOnDb("Life Chart Daily Completed", datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    recordStatsOnDb("Life Chart Daily Processed", processed)
    recordStatsOnDb("Life Chart Daily Empty", empty_records)
    recordStatsOnDb("Pension & Life Charts Migration Started", datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    migrateNewDBToOld()
    recordStatsOnDb("Pension & Life Charts Migration Completed", datetime.now().strftime('%Y-%m-%d %H:%M:%S'))