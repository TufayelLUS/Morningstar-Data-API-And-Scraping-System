...
def addToDb(data):
    ...
    client = MongoClient(mongo_connection_string)
    db = client['morningstar']
    collection = db['etf_screener_data']
    # drop old data
...
def scrapeFunds():
    ...
    data = []
    fails = 0
    with open('etf_sec_ids.txt', 'w+', encoding='utf-8') as f:
        while True:
